...
void loop() {
  for (int scan = 1; scan < 23; scan++){
    ...
    if (scan ==11) digitalWrite (LED11Pin, HIGH); 
    if (scan ==12) digitalWrite (LED12Pin, HIGH); 
    if (scan ==13) digitalWrite (LED11Pin, HIGH);
    if (scan ==14) digitalWrite (LED10Pin, HIGH);
    if (scan ==15) digitalWrite (LED9Pin, HIGH);
    if (scan ==16) digitalWrite (LED8Pin, HIGH);
    if (scan ==17) digitalWrite (LED7Pin, HIGH);
    if (scan ==18) digitalWrite (LED6Pin, HIGH);
    if (scan ==19) digitalWrite (LED5Pin, HIGH);
    if (scan ==20) digitalWrite (LED4Pin, HIGH);
    if (scan ==21) digitalWrite (LED3Pin, HIGH);
    if (scan ==22) digitalWrite (LED2Pin, HIGH);
  delay(500);
  }
}
