...
void RunSign()
{
  for (int row = 7; row > 0; row--)
  {
    ...
    {
    if bitRead(bitmap[character][row],5) digitalWrite(dataPin, HIGH); else digitalWrite(dataPin, LOW);
    digitalWrite(clockPin, HIGH); digitalWrite(clockPin, LOW);
    if bitRead(bitmap[character][row],4) digitalWrite(dataPin, HIGH); else digitalWrite(dataPin, LOW);
    digitalWrite(clockPin, HIGH); digitalWrite(clockPin, LOW);
    if bitRead(bitmap[character][row],3) digitalWrite(dataPin, HIGH); else digitalWrite(dataPin, LOW);
    digitalWrite(clockPin, HIGH); digitalWrite(clockPin, LOW);
    if bitRead(bitmap[character][row],2) digitalWrite(dataPin, HIGH); else digitalWrite(dataPin, LOW);
    digitalWrite(clockPin, HIGH); digitalWrite(clockPin, LOW);
    if bitRead(bitmap[character][row],1) digitalWrite(dataPin, HIGH); else digitalWrite(dataPin, LOW);
    digitalWrite(clockPin, HIGH); digitalWrite(clockPin, LOW);
    if bitRead(bitmap[character][row],0) digitalWrite(dataPin, HIGH); else digitalWrite(dataPin, LOW);
    digitalWrite(clockPin, HIGH); digitalWrite(clockPin, LOW);
//    if bitRead(character,0) shiftOut(dataPin, clockPin, MSBFIRST, bitmap[character][row]);
//    else shiftOut(gdataPin, clockPin, MSBFIRST, bitmap[character][row]);
    }
    //latch the data
