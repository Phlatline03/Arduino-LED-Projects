...
void RefreshDisplay()
{
  for (int row = 0; row < 8; row++) {
    ...
    digitalWrite(rowEnable, HIGH); //Turn off display
    digitalWrite(rclockPin, LOW);
    //-- Shift out to each matrix (zone is 8 columns represented by one matrix)
    for (int zone = maxZoneIndex; zone >= 0; zone--) {
      shiftOut(gdataPin, clockPin, MSBFIRST, bitmap[row][zone]);
    }
    for (int zone = maxZoneIndex; zone >= 0; zone--) {
    ...
    digitalWrite(rowEnable, LOW);  // Turn on display
...
//      for (int row=7; row>0; row--)  //still right shift
      {
 //       for (int zone=numZones; zone > 0; zone--)
     ...
     //     bitmap[row][zone] = bitmap[row][zone] << 1;   //no shift
